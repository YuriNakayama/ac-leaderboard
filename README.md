...
- **ユーザー認証**: AWS Cognitoを使用した認証機能
  ...
  - Cognito Identity Poolを使用したAWSリソースへのアクセス制御
- **リーダーボード**: リアルタイムでスコアを表示
- **スコアアップロード**: CSVファイルからスコアをアップロード
- **管理機能**:
  - 管理者によるスコア編集
  - 管理者によるユーザー管理(有効化・無効化・削除)
...
2. User Poolを選択
3. `Groups`タブから`admin`グループを選択
4. `Add user to group`をクリックしてユーザーを追加

### 管理者機能

管理者権限を持つユーザーは、以下の機能を利用できます:
...
**注意**: ユーザー管理機能は、Cognito Identity Poolを通じて管理者グループに割り当てられた特別なIAMロールを使用しています。このロールには、Cognito Admin API(`AdminEnableUser`, `AdminDisableUser`, `AdminDeleteUser`, `ListUsers`など)へのアクセス権限が付与されています。
